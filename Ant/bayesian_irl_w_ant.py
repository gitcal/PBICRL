...
class BIRL:
    ...
    def calc_ll_pref(self, rew_tuple,  trajectories, preferences):
        ...
        for k in range(len(preferences)):# iterate over 3 categories of pairwise preferences
            for itr in range(self.num_samples):
                ...
                if self.norm_flag:
                    if k==0:
                        log_sum += self.s*(self.beta1 * np.mean(rew1)) - logsumexp([self.s*(self.beta1 * np.mean(rew1)), self.s*self.beta1 * np.mean(rew2)]) 
                    if k==1:
                        log_sum += self.s*(self.beta2 * np.mean(rew1)) - logsumexp([self.s*(self.beta2 * np.mean(rew1)), self.s*self.beta2 * np.mean(rew2)]) 
                    if k==2:
                        log_sum += self.s*(self.beta3 * np.mean(rew1)) - logsumexp([self.s*(self.beta3 * np.mean(rew1)), self.s*self.beta3 * np.mean(rew2)])
                    if k==3:
    ...
    def initial_solution_bern_cnstr(self):
        ...
        pen_rew = np.random.uniform(-100, -10)#, self.env.num_states)
        x_loc_new = 5*np.random.randn()
        W_ind_new = np.random.randint(2)
    ...
    def run_mcmc_bern_constraint(self, samples, W_fix, stdev1, stdev2):
        ...
        self.chain_W_ind = np.zeros((num_samples, 1)) #store rewards found via BIRL here, preallocate for speed
        self.chain_rew = np.zeros((num_samples, 1)) 
        self.chain_x_loc = np.zeros((num_samples, 1)) 
    ...
    def get_mean_solution(self, burn_frac=0.1, skip_rate=1):
        ...
        '''
        Chain_W_ind = self.chain_W_ind#[burn_frac::skip_rate]
