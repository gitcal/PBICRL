...
parser.add_argument('--fl_name', type=str, default='results2_IRL_')
...
def main():
    ...
    env = Point('name', 20, -1, 20, -1, np.array([20.0,20.0]), np.array([5.5,14.5]), np.array([14.5,5.5]),0,0)
    ...
    for iters in range(iterations):
        ...
        # random.shuffle(bad_vvbad_pairs)
        # random.shuffle(good_vvbad_pairs)
        ...
        d13 = []
        for ii in range(1):
            birl = bayesian_irl_w_IRL.BIRL(env, traj_feat, pref_pairs, boltz_beta1, boltz_beta2, boltz_beta3, N_demonstrations, s, m1, m2, m3, norm_flag)
            birl.run_mcmc_bern_constraint(num_steps, env.w_nom, stdev)
            acc_rate = birl.accept_rate
            print('acceptance rate {}'.format(acc_rate)) 
            chain_w = birl.get_mean_solution(burn_frac=0.2, skip_rate=1)
            map_w = birl.get_map_solution()
            res_dict[iters,'weights'] = map_w
            res_dict[iters,'mean_w'] = chain_w[-500:,:]
